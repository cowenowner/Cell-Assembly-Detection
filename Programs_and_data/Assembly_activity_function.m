...
for n=1:length(as)
    ...
    switch act_count
        case 'full'
            if iscell(spM)
                [activity]=DetectAssembly(spM{ix} ,as{n});  %  only full assembly occurrences are considered
            else
                [activity]=DetectAssembly(spM ,as{n},tb);  %  only full assembly occurrences are considered
            end
        case 'partial'
            if iscell(spM)
                [activity]=DetectAssembly_additive(spM{ix} ,as{n});  %  only full assembly occurrences are considered
            else               
                [activity]=DetectAssembly_additive(spM ,as{n},tb);  % partial activations are also took into account
            end
        case 'combined'
            if iscell(spM)
                [activity]=DetectAssembly_additive_weighted(spM{ix} ,as{n});  %  only full assembly occurrences are considered           
            else
